...
import {
  ...
  K2_EXPECTED_SOURCE_FILE_HASH,
  L2_EXPECTED_SOURCE_FILE_HASH,
  M2_EXPECTED_SOURCE_FILE_HASH,
  N2_EXPECTED_SOURCE_FILE_HASH,
...
const k2Approval = {
  ...
  stage: "K-2",
  target_part: "4",
  test_file_hash: K2_EXPECTED_SOURCE_FILE_HASH,
  date_from: "2026-06-07",
  date_to: "2026-06-12",
  max_rows: 500,
  expectedInsertedRows: 500,
...
const kSeriesApprovals = [k2Approval, k3Approval, k4Approval];
...
const l2Approval = {
  ...
  stage: "L-2",
  target_part: "5",
  test_file_hash: L2_EXPECTED_SOURCE_FILE_HASH,
  date_from: "2026-06-01",
  date_to: "2026-06-06",
  max_rows: 500,
  expectedInsertedRows: 500,
...
const lSeriesApprovals = [l2Approval, l3Approval, l4Approval, l5Approval];
...
const m2Approval = {
  ...
  stage: "M-2",
  target_part: "6",
  test_file_hash: M2_EXPECTED_SOURCE_FILE_HASH,
  date_from: "2026-06-01",
  date_to: "2026-06-06",
  max_rows: 500,
  expectedInsertedRows: 500,
...
const mSeriesApprovals = [m2Approval, m3Approval, m4Approval];
...
const n2Approval = {
  ...
  stage: "N-2",
  target_part: "7",
  test_file_hash: N2_EXPECTED_SOURCE_FILE_HASH,
  date_from: "2026-06-01",
  date_to: "2026-06-06",
  max_rows: 500,
  expectedInsertedRows: 500,
...
const nSeriesApprovals = [n2Approval, n3Approval, n4Approval, n5Approval];
...
describe("limited apply approval gate", () => {
  it("keeps stage caps and explicit-period policy centralized", () => {
    expect(LIMITED_APPLY_STAGE_POLICIES).toEqual({
      ...
      "K-2": { maxRows: 500, requiresExplicitPeriod: true },
      "K-3": { maxRows: 500, requiresExplicitPeriod: true },
      "K-4": { maxRows: 338, requiresExplicitPeriod: true },
      "L-2": { maxRows: 500, requiresExplicitPeriod: true },
      "L-3": { maxRows: 500, requiresExplicitPeriod: true },
      "L-4": { maxRows: 500, requiresExplicitPeriod: true },
      "L-5": { maxRows: 46, requiresExplicitPeriod: true },
      "M-2": { maxRows: 500, requiresExplicitPeriod: true },
      "M-3": { maxRows: 500, requiresExplicitPeriod: true },
      "M-4": { maxRows: 359, requiresExplicitPeriod: true },
      "N-2": { maxRows: 500, requiresExplicitPeriod: true },
      "N-3": { maxRows: 500, requiresExplicitPeriod: true },
      "N-4": { maxRows: 500, requiresExplicitPeriod: true },
      "N-5": { maxRows: 420, requiresExplicitPeriod: true },
    });
  });
  ...
  it("recognizes only explicit K-series part-4 limited apply stages", () => {
    expect(isLimitedApplyStage("K-2")).toBe(true);
    expect(getLimitedApplyStageConfig("K-2")).toMatchObject({
      stage: "K-2",
      expectedTargetPartCode: "4",
      expectedWorkflowGate: "K-2",
      approvalFileName: "k2_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 0,
      expectedInsertCandidates: 1338,
      expectedNoChangeRows: 0,
      requiresExplicitPeriod: true,
    ...
    expect(isLimitedApplyStage("K-3")).toBe(true);
    expect(getLimitedApplyStageConfig("K-3")).toMatchObject({
      stage: "K-3",
      expectedTargetPartCode: "4",
      expectedWorkflowGate: "K-3",
      approvalFileName: "k3_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 500,
      expectedInsertCandidates: 838,
      expectedNoChangeRows: 500,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: K2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("K-4")).toBe(true);
    expect(getLimitedApplyStageConfig("K-4")).toMatchObject({
      stage: "K-4",
      expectedTargetPartCode: "4",
      expectedWorkflowGate: "K-4",
      approvalFileName: "k4_limited_apply_approval.json",
      expectedMaxRows: 338,
      expectedExistingScopedRows: 1000,
      expectedInsertCandidates: 338,
      expectedNoChangeRows: 1000,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: K2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("K-0")).toBe(false);
    expect(isLimitedApplyStage("K-1")).toBe(false);
    expect(isLimitedApplyStage("K-5")).toBe(false);
    expect(isLimitedApplyStage("K-*")).toBe(false);
    expect(getLimitedApplyStageConfig("K-5")).toBeNull();
  });

  it("recognizes only explicit L-series part-5 limited apply stages", () => {
    expect(isLimitedApplyStage("L-2")).toBe(true);
    expect(getLimitedApplyStageConfig("L-2")).toMatchObject({
      stage: "L-2",
      expectedTargetPartCode: "5",
      expectedWorkflowGate: "L-2",
      approvalFileName: "l2_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 0,
      expectedInsertCandidates: 1546,
      expectedNoChangeRows: 0,
      requiresExplicitPeriod: true,
    ...
    expect(isLimitedApplyStage("L-3")).toBe(true);
    expect(getLimitedApplyStageConfig("L-3")).toMatchObject({
      stage: "L-3",
      expectedTargetPartCode: "5",
      expectedWorkflowGate: "L-3",
      approvalFileName: "l3_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 500,
      expectedInsertCandidates: 1046,
      expectedNoChangeRows: 500,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: L2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("L-4")).toBe(true);
    expect(getLimitedApplyStageConfig("L-4")).toMatchObject({
      stage: "L-4",
      expectedTargetPartCode: "5",
      expectedWorkflowGate: "L-4",
      approvalFileName: "l4_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 1000,
      expectedInsertCandidates: 546,
      expectedNoChangeRows: 1000,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: L2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("L-5")).toBe(true);
    expect(getLimitedApplyStageConfig("L-5")).toMatchObject({
      stage: "L-5",
      expectedTargetPartCode: "5",
      expectedWorkflowGate: "L-5",
      approvalFileName: "l5_limited_apply_approval.json",
      expectedMaxRows: 46,
      expectedExistingScopedRows: 1500,
      expectedInsertCandidates: 46,
      expectedNoChangeRows: 1500,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: L2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("L-0")).toBe(false);
    expect(isLimitedApplyStage("L-1")).toBe(false);
    expect(isLimitedApplyStage("L-6")).toBe(false);
    expect(isLimitedApplyStage("L-*")).toBe(false);
    expect(getLimitedApplyStageConfig("L-6")).toBeNull();
  });

  it("recognizes only explicit M-series part-6 limited apply stages", () => {
    expect(isLimitedApplyStage("M-2")).toBe(true);
    expect(getLimitedApplyStageConfig("M-2")).toMatchObject({
      stage: "M-2",
      expectedTargetPartCode: "6",
      expectedWorkflowGate: "M-2",
      approvalFileName: "m2_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 0,
      expectedInsertCandidates: 1359,
      expectedNoChangeRows: 0,
      requiresExplicitPeriod: true,
    ...
    expect(isLimitedApplyStage("M-3")).toBe(true);
    expect(getLimitedApplyStageConfig("M-3")).toMatchObject({
      stage: "M-3",
      expectedTargetPartCode: "6",
      expectedWorkflowGate: "M-3",
      approvalFileName: "m3_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 500,
      expectedInsertCandidates: 859,
      expectedNoChangeRows: 500,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: M2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("M-4")).toBe(true);
    expect(getLimitedApplyStageConfig("M-4")).toMatchObject({
      stage: "M-4",
      expectedTargetPartCode: "6",
      expectedWorkflowGate: "M-4",
      approvalFileName: "m4_limited_apply_approval.json",
      expectedMaxRows: 359,
      expectedExistingScopedRows: 1000,
      expectedInsertCandidates: 359,
      expectedNoChangeRows: 1000,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: M2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("M-0")).toBe(false);
    expect(isLimitedApplyStage("M-1")).toBe(false);
    expect(isLimitedApplyStage("M-5")).toBe(false);
    expect(isLimitedApplyStage("M-*")).toBe(false);
    expect(getLimitedApplyStageConfig("M-5")).toBeNull();
  });

  it("recognizes only explicit N-series part-7 limited apply stages", () => {
    expect(isLimitedApplyStage("N-2")).toBe(true);
    expect(getLimitedApplyStageConfig("N-2")).toMatchObject({
      stage: "N-2",
      expectedTargetPartCode: "7",
      expectedWorkflowGate: "N-2",
      approvalFileName: "n2_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 0,
      expectedInsertCandidates: 1920,
      expectedNoChangeRows: 0,
      requiresExplicitPeriod: true,
    ...
    expect(isLimitedApplyStage("N-3")).toBe(true);
    expect(getLimitedApplyStageConfig("N-3")).toMatchObject({
      stage: "N-3",
      expectedTargetPartCode: "7",
      expectedWorkflowGate: "N-3",
      approvalFileName: "n3_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 500,
      expectedInsertCandidates: 1420,
      expectedNoChangeRows: 500,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: N2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("N-4")).toBe(true);
    expect(getLimitedApplyStageConfig("N-4")).toMatchObject({
      stage: "N-4",
      expectedTargetPartCode: "7",
      expectedWorkflowGate: "N-4",
      approvalFileName: "n4_limited_apply_approval.json",
      expectedMaxRows: 500,
      expectedExistingScopedRows: 1000,
      expectedInsertCandidates: 920,
      expectedNoChangeRows: 1000,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: N2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("N-5")).toBe(true);
    expect(getLimitedApplyStageConfig("N-5")).toMatchObject({
      stage: "N-5",
      expectedTargetPartCode: "7",
      expectedWorkflowGate: "N-5",
      approvalFileName: "n5_limited_apply_approval.json",
      expectedMaxRows: 420,
      expectedExistingScopedRows: 1500,
      expectedInsertCandidates: 420,
      expectedNoChangeRows: 1500,
      requiresExplicitPeriod: true,
      expectedSourceFileHash: N2_EXPECTED_SOURCE_FILE_HASH,
    });
    expect(isLimitedApplyStage("N-0")).toBe(false);
    expect(isLimitedApplyStage("N-1")).toBe(false);
    expect(isLimitedApplyStage("N-6")).toBe(false);
    expect(isLimitedApplyStage("N-*")).toBe(false);
    expect(getLimitedApplyStageConfig("N-6")).toBeNull();
  });
  ...
  it("accepts the K-series exact insert-only approval shapes", () => {
    for (const approval of kSeriesApprovals) {
      expect(validateLimitedApplyApproval(approval)).toEqual({
        ok: true,
        blockedReasons: [],
        approval,
  ...
  it("accepts the L-series exact insert-only approval shapes", () => {
    for (const approval of lSeriesApprovals) {
      expect(validateLimitedApplyApproval(approval)).toEqual({
        ok: true,
        blockedReasons: [],
        approval,
  ...
  it("accepts the M-series exact insert-only approval shapes", () => {
    for (const approval of mSeriesApprovals) {
      expect(validateLimitedApplyApproval(approval)).toEqual({
        ok: true,
        blockedReasons: [],
        approval,
  ...
  it("accepts the N-series exact insert-only approval shapes", () => {
    for (const approval of nSeriesApprovals) {
      expect(validateLimitedApplyApproval(approval)).toEqual({
        ok: true,
        blockedReasons: [],
        approval,
  ...
  it("blocks K-series approvals outside maxRows, insert-only, exact hash, exact period, and workflow constraints", () => {
    for (const approval of kSeriesApprovals) {
      const wrongWorkflowGate = validateLimitedApplyApproval({
        ...approval,
        workflowGate: `${approval.workflowGate}-B`,
  ...
  it("blocks L-series approvals outside maxRows, insert-only, exact hash, exact period, and workflow constraints", () => {
    for (const approval of lSeriesApprovals) {
      const wrongWorkflowGate = validateLimitedApplyApproval({
        ...approval,
        workflowGate: `${approval.workflowGate}-B`,
  ...
  it("blocks M-series approvals outside maxRows, insert-only, exact hash, exact period, and workflow constraints", () => {
    for (const approval of mSeriesApprovals) {
      const wrongWorkflowGate = validateLimitedApplyApproval({
        ...approval,
        workflowGate: `${approval.workflowGate}-B`,
  ...
  it("blocks N-series approvals outside maxRows, insert-only, exact hash, exact period, and workflow constraints", () => {
    for (const approval of nSeriesApprovals) {
      const wrongWorkflowGate = validateLimitedApplyApproval({
        ...approval,
        workflowGate: `${approval.workflowGate}-B`,
  ...
  it("blocks K-series approvals outside the exact expected count contract", () => {
    for (const approval of kSeriesApprovals) {
      const missingExpectedInsertedRows = validateLimitedApplyApproval({
        ...approval,
        expectedInsertedRows: undefined,
  ...
  it("blocks L-series approvals outside the exact expected count contract", () => {
    for (const approval of lSeriesApprovals) {
      const wrongExpectedInsertedRows = validateLimitedApplyApproval({
        ...approval,
        expectedInsertedRows: undefined,
      });
      const wrongPrimaryRows = validateLimitedApplyApproval({
        ...approval,
        source_preview: {
          ...approval.source_preview,
          primaryScopeRows: approval.source_preview.primaryScopeRows - 1,
      ...
      expect(wrongExpectedInsertedRows.ok).toBe(false);
      expect(wrongExpectedInsertedRows.blockedReasons).toContain("APPROVAL_EXPECTED_INSERTED_ROWS_MISMATCH");
      expect(wrongPrimaryRows.ok).toBe(false);
      expect(wrongPrimaryRows.blockedReasons).toContain("APPROVAL_PRIMARY_SCOPE_ROWS_MISMATCH");
      expect(wrongExistingRows.ok).toBe(false);
      expect(wrongExistingRows.blockedReasons).toContain("APPROVAL_EXISTING_SCOPED_ROWS_MISMATCH");
      expect(wrongInsertCandidates.ok).toBe(false);
      expect(wrongInsertCandidates.blockedReasons).toContain("APPROVAL_INSERT_CANDIDATES_MISMATCH");
  ...
  it("blocks M-series approvals outside the exact expected count contract", () => {
    for (const approval of mSeriesApprovals) {
      const wrongExpectedInsertedRows = validateLimitedApplyApproval({
        ...approval,
        expectedInsertedRows: undefined,
  ...
  it("blocks N-series approvals outside the exact expected count contract", () => {
    for (const approval of nSeriesApprovals) {
      const wrongExpectedInsertedRows = validateLimitedApplyApproval({
        ...approval,
        expectedInsertedRows: undefined,
  ...
  it("blocks K-series dryRun=false entry when the local approval file is missing", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "cn-sales-k-series-approval-"));
    const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(tempDir);

    try {
      for (const stage of ["K-2", "K-3", "K-4"] as const) {
        await expect(loadLimitedApplyApproval(stage)).resolves.toEqual({
          ok: false,
          blockedReasons: ["APPROVAL_FILE_MISSING_OR_UNREADABLE"],
        });
  ...
  it("blocks L-series dryRun=false entry when the local approval file is missing", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "cn-sales-l-series-approval-"));
    const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(tempDir);

    try {
      for (const stage of ["L-2", "L-3", "L-4", "L-5"] as const) {
        await expect(loadLimitedApplyApproval(stage)).resolves.toEqual({
          ok: false,
          blockedReasons: ["APPROVAL_FILE_MISSING_OR_UNREADABLE"],
        });
  ...
  it("creates aggregate-only K-series diagnostics without exposing raw row content", () => {
    const rows = Array.from({ length: 1338 }, (_, index) => row(index + 1, "2026-06-07"));
    const syncRows = Array.from({ length: 1338 }, (_, index) => syncRow(index + 1));
    const existingRows = Array.from({ length: 1000 }, (_, index) => syncRow(index + 1));
    const diagnostics = createLimitedApplySelectionDiagnostics({
      stage: "K-4",
      rows,
    ...
    expect(diagnostics).toMatchObject({
      ...
      maxRows: 338,
      candidateRows: 338,
      selectedRowsDryRunEquivalent: 338,
      candidateDigestMatchesSelector: true,
      orderDigestMatchesSelector: true,
      rawRowsReturned: false,
    });
  ...
  it("creates aggregate-only L-series diagnostics without exposing raw row content", () => {
    const rows = Array.from({ length: 1546 }, (_, index) => row(index + 1, "2026-06-01"));
    const syncRows = Array.from({ length: 1546 }, (_, index) => syncRow(index + 1));
    const existingRows = Array.from({ length: 1500 }, (_, index) => syncRow(index + 1));
    const diagnostics = createLimitedApplySelectionDiagnostics({
      stage: "L-5",
      rows,
      syncRows,
    ...
    expect(diagnostics).toMatchObject({
      ...
      maxRows: 46,
      candidateRows: 46,
      selectedRowsDryRunEquivalent: 46,
      candidateDigestMatchesSelector: true,
      orderDigestMatchesSelector: true,
      rawRowsReturned: false,
    });
  ...
  it("allows K-series stages when aggregate dry-run diff matches each part-4 6-12 batch scope", () => {
    for (const approval of kSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("allows L-series stages when aggregate dry-run diff matches each part-5 1-6 batch scope", () => {
    for (const approval of lSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks M-series dryRun=false entry when the local approval file is missing", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "cn-sales-m-series-approval-"));
    const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(tempDir);

    try {
      for (const stage of ["M-2", "M-3", "M-4"] as const) {
        await expect(loadLimitedApplyApproval(stage)).resolves.toEqual({
          ok: false,
          blockedReasons: ["APPROVAL_FILE_MISSING_OR_UNREADABLE"],
        });
  ...
  it("blocks N-series dryRun=false entry when the local approval file is missing", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "cn-sales-n-series-approval-"));
    const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(tempDir);

    try {
      for (const stage of ["N-2", "N-3", "N-4", "N-5"] as const) {
        await expect(loadLimitedApplyApproval(stage)).resolves.toEqual({
          ok: false,
          blockedReasons: ["APPROVAL_FILE_MISSING_OR_UNREADABLE"],
        });
  ...
  it("allows M-series stages when aggregate dry-run diff matches each part-6 1-6 batch scope", () => {
    for (const approval of mSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("allows N-series stages when aggregate dry-run diff matches each part-7 1-6 batch scope", () => {
    for (const approval of nSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks K-series stages without an explicit request period scope", () => {
    for (const approval of kSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks L-series stages without an explicit request period scope", () => {
    for (const approval of lSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks M-series stages without an explicit request period scope", () => {
    for (const approval of mSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks N-series stages without an explicit request period scope", () => {
    for (const approval of nSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks K-series stages when the request, part, hash, or dry-run counts drift before write", () => {
    for (const approval of kSeriesApprovals) {
      const requestMismatch = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
      ...
      const wrongHash = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: J2_EXPECTED_SOURCE_FILE_HASH,
        selectedPartCode: "4",
        syncDiff: diffPlan({
          partCode: "4",
          dateFrom: "2026-06-07",
  ...
  it("blocks L-series stages when the request, part, hash, or dry-run counts drift before write", () => {
    for (const approval of lSeriesApprovals) {
      const requestMismatch = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
      ...
      const wrongHash = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: K2_EXPECTED_SOURCE_FILE_HASH,
        selectedPartCode: "5",
        syncDiff: diffPlan({
          partCode: "5",
          dateFrom: "2026-06-01",
  ...
  it("blocks M-series stages when the request, part, hash, or dry-run counts drift before write", () => {
    for (const approval of mSeriesApprovals) {
      const requestMismatch = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
      ...
      const wrongHash = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: L2_EXPECTED_SOURCE_FILE_HASH,
        selectedPartCode: "6",
        syncDiff: diffPlan({
          partCode: "6",
          dateFrom: "2026-06-01",
  ...
  it("blocks K-series stages when update, delete, warning, or error rows appear before write", () => {
    for (const approval of kSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks L-series stages when update, delete, warning, or error rows appear before write", () => {
    for (const approval of lSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks M-series stages when update, delete, warning, or error rows appear before write", () => {
    for (const approval of mSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
  ...
  it("blocks N-series stages when the request, part, hash, or dry-run counts drift before write", () => {
    for (const approval of nSeriesApprovals) {
      const requestMismatch = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
      ...
      const wrongHash = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: M2_EXPECTED_SOURCE_FILE_HASH,
        selectedPartCode: "7",
        syncDiff: diffPlan({
          partCode: "7",
          dateFrom: "2026-06-01",
  ...
  it("blocks N-series stages when update, delete, warning, or error rows appear before write", () => {
    for (const approval of nSeriesApprovals) {
      const result = validateLimitedApplyPreconditions({
        approval,
        sourceFileHash: approval.test_file_hash,
