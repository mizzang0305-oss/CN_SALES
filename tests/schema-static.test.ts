...
describe("cn_sales schema migration static checks", () => {
  ...
  it("keeps grants and storage policy least-privilege for Phase 3-A", () => {
    ...
    expect(migrationSql).toContain("values ('cn-sales-ledgers', 'cn-sales-ledgers', false)");
  });
  ...
  it("does not directly create pgcrypto in the migration", () => {
    expect(migrationSql).not.toMatch(/^\s*create\s+extension\s+if\s+not\s+exists\s+pgcrypto;/im);
